...
/**
Bot is an abstract class for any "player" or "robot" subclass
that will be controlled by a path-finding and/or moving  algorithm().
<p>
...
/** 
Restore bot to initial simulation model.  Allows algorithm to be
simulated (run) again with same model (setup). 
*/
...
/**
Behaviors (actions) to simulate on each step in the
algorithm's simulation.  Subclass must determine how the bot 
should move.  Probably makes calls to moveTo(...)
*/
